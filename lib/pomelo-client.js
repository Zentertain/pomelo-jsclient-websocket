(function() {
  ...
  if(typeof(window) != "undefined" && typeof(sys) != 'undefined' && sys.localStorage && typeof(sys.localStorage.getItem) === "function" && typeof(sys.localStorage.setItem) === "function") {
    windowLocalStorage = sys.localStorage;
  } else {
    windowLocalStorage = memoryLocalStorage;
  }
  ...
  var connect = function(params, url, cb) {
    ...
    if(windowLocalStorage && windowLocalStorage.getItem('protos') && protoVersion === 0) {
      var protos = JSON.parse(windowLocalStorage.getItem('protos'));
    ...
    var onopen = function(event) {
      ...
      reset();
      var obj = Package.encode(Package.TYPE_HANDSHAKE, textEncoder.encode(JSON.stringify(handshakeBuffer)));
      send(obj);
    };
  ...
  var deCompose = function(msg) {
    ...
    if(protobuf && serverProtos[route]) {
      ...
      return JSON.parse(textDecoder.decode(msg.body));
    }
  ...
  var initData = function(data) {
    ...
    if(protos) {
      ...
      clientProtos = protos.client || {};
        ...
        windowLocalStorage.setItem('protos', JSON.stringify(protos));
